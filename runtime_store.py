...
def resolve_runtime_store_backend(target: Path) -> str:
    configured = str(os.getenv("NEXUS_HIVE_RUNTIME_STORE_BACKEND", "")).strip().lower()
...
def append_runtime_event(event: dict[str, Any]) -> None:
    ...
    backend = resolve_runtime_store_backend(target)
    if backend == "jsonl":
        target.parent.mkdir(parents=True, exist_ok=True)
...
def build_runtime_store_summary(limit: int = 25) -> dict[str, Any]:
    ...
    with ensure_sqlite_store(target) as connection:
        count, last_event_at = connection.execute(
        ...
        for row in reversed(rows):
            try:
                recent_events.append(json.loads(str(row[0])))
            except json.JSONDecodeError:
                continue
