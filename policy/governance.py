...
def _run_single_row_preview(sql: str) -> Dict[str, Any]:
    preview = execute_sql_preview(sql)
...
def _quality_gate_violation_snapshot() -> Dict[str, int]:
    row = _run_single_row_preview(
        """
        WITH quality_gate_snapshot AS (
            SELECT
                (SELECT COUNT(*)
                 FROM sales
                 WHERE transaction_id IS NULL
                    ...
                    OR net_revenue IS NULL) AS sales_primary_fields_not_null,
                (SELECT COUNT(*)
                 FROM sales s
                 LEFT JOIN products p ON s.product_id = p.product_id
                 WHERE p.product_id IS NULL) AS sales_product_fk_integrity,
                (SELECT COUNT(*)
                 FROM sales s
                 LEFT JOIN regions r ON s.region_id = r.region_id
                 WHERE r.region_id IS NULL) AS sales_region_fk_integrity,
                (SELECT COUNT(*)
                 FROM (
                     ...
                     HAVING COUNT(*) > 1
                 ) dupes) AS transaction_id_uniqueness
        )
        SELECT *
        FROM quality_gate_snapshot
        """
    )
    return {
        "sales_primary_fields_not_null": int(
            float(row.get("sales_primary_fields_not_null", 0) or 0)
        ),
        "sales_product_fk_integrity": int(float(row.get("sales_product_fk_integrity", 0) or 0)),
        "sales_region_fk_integrity": int(float(row.get("sales_region_fk_integrity", 0) or 0)),
        "transaction_id_uniqueness": int(float(row.get("transaction_id_uniqueness", 0) or 0)),
    }


def build_quality_gate(table_profiles: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    table_profiles = table_profiles or build_table_profiles()
    ...
    violation_snapshot = (
        _quality_gate_violation_snapshot()
        if not missing_tables
        else {
            "sales_primary_fields_not_null": 0,
            "sales_product_fk_integrity": 0,
            "sales_region_fk_integrity": 0,
            "transaction_id_uniqueness": 0,
        }
    )
...
def run_gold_eval_suite(
    strategy: str = "heuristic", *, execute_previews: bool = True
) -> Dict[str, Any]:
    items = []
    passed = 0
    ...
    for case in GOLD_EVAL_CASES:
        ...
        if policy_verdict["decision"] != "deny" and execute_previews:
            ...
            execution = {
                ...
                "preview": [],
                "elapsed_ms": 0,
                "execution_mode": "skipped-live-brief",
            }
...
def build_warehouse_brief() -> Dict[str, Any]:
    ...
    cache_key = "|".join(
        [
            ...
            active_adapter.contract.execution_mode,
            str(os.getenv("DATABRICKS_HOST", "")),
            str(os.getenv("DATABRICKS_SCHEMA", "")),
            str(os.getenv("SNOWFLAKE_ACCOUNT", "")),
            str(os.getenv("SNOWFLAKE_SCHEMA", "")),
        ]
    )
    cached = _WAREHOUSE_BRIEF_CACHE.get(cache_key)
    if (
        cached is not None
        and active_adapter.contract.execution_mode in {"snowflake-live", "databricks-live"}
        and (time.monotonic() - float(cached["cached_at_monotonic"]))
        < _WAREHOUSE_BRIEF_CACHE_TTL_SEC
    ):
        return cast(Dict[str, Any], cached["payload"])

    table_profiles = build_table_profiles()
    quality_gate = build_quality_gate(table_profiles=table_profiles)
    date_window = fetch_date_window()
    recent_audits = list_recent_query_audits(limit=5)
    gold_eval = build_gold_eval_pack()
    gold_eval_run = run_gold_eval_suite(
        execute_previews=active_adapter.contract.execution_mode
        not in {"snowflake-live", "databricks-live"}
    )
    ...
    payload = {
        ...
        "gold_eval_live_preview_mode": (
            "deferred"
            if active_adapter.contract.execution_mode in {"snowflake-live", "databricks-live"}
            else "inline"
        ),
    ...
    if active_adapter.contract.execution_mode in {"snowflake-live", "databricks-live"}:
        _WAREHOUSE_BRIEF_CACHE[cache_key] = {
