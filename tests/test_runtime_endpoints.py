...
def test_health_and_meta_expose_runtime_diagnostics() -> None:
    ...
    assert (
        health_payload["links"]["warehouse_mode_switchboard"]
    ...
    assert warehouse_switchboard_payload["schema"] == "nexus-hive-warehouse-mode-switchboard-v1"
    assert warehouse_switchboard_payload["links"]["runtime_brief"] == "/api/runtime/brief"
    assert len(warehouse_switchboard_payload["targets"]) == 3
