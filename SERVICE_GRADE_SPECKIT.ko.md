...
- 이번 iteration 목표: reviewer가 첫 화면과 API surface만 봐도 `질문 -> SQL -> 실행 -> 시각화 -> 검토` 계약을 이해하게 만든다.
...
- landing UI에 runtime brief panel을 올려 operator가 질문 전에 readiness를 확인하게 만든다.
...
- `/health`, `/api/meta`, `/api/runtime/brief`, `/api/schema/answer` contract가 일관된다.
- 첫 화면에서 answer schema, model, warehouse readiness, review flow가 즉시 보인다.
