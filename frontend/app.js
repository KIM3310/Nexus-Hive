...
const API_BASE = String(document.body?.dataset?.apiBase || window.NEXUS_API_BASE || '')
    .trim()
    .replace(/\/$/, '');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const briefArchitectureFlow = document.getElementById('brief-review-flow');
    ...
    const architecturePackHeadline = document.getElementById('reviewpack-headline');
    const architecturePackBadge = document.getElementById('reviewpack-badge');
    const architecturePackReady = document.getElementById('reviewpack-ready');
    const architecturePackRoutes = document.getElementById('reviewpack-routes');
    const architecturePackSchema = document.getElementById('reviewpack-schema');
    const architecturePackRetry = document.getElementById('reviewpack-retry');
    const architecturePackPromises = document.getElementById('reviewpack-promises');
    const architecturePackBoundary = document.getElementById('reviewpack-boundary');
    const architecturePackSequence = document.getElementById('reviewpack-sequence');
    const architecturePackWatchouts = document.getElementById('reviewpack-watchouts');
    ...
    const copyArchitectureRoutesBtn = document.getElementById('copy-review-routes-btn');
    ...
    function renderStoryboard() {
        ...
        storyboardRoutes.innerText = `${architectureRoutes.length} routes`;
    ...
    async function loadRuntimeBrief() {
        if (shouldPreferRecordedReview()) {
            ...
            renderBriefList(briefArchitectureFlow, payload.architecture_flow || []);
        ...
        try {
            const response = await fetch(apiUrl('/api/runtime/brief'));
            if (!response.ok) {
                throw new Error(`Runtime brief request failed with ${response.status}`);
            }

            const payload = await readJsonOrThrow(response, 'Runtime brief');
            const diagnostics = payload.diagnostics || {};
            const reportContract = payload.report_contract || {};
            const evidenceCounts = payload.evidence_counts || {};
    ...
    async function loadArchitecturePack() {
        if (shouldPreferRecordedReview()) {
            ...
            architecturePackHeadline.innerText = payload.headline;
            architecturePackBadge.innerText = 'RECORDED';
            architecturePackReady.innerText = proofBundle.warehouse_ready ? 'Auditable' : 'Degraded';
            architecturePackRoutes.innerText = `${(proofBundle.architecture_routes || []).length} routes`;
            architecturePackSchema.innerText = answerContract.schema || 'Unavailable';
            architecturePackRetry.innerText = `${proofBundle.retry_budget || 0} retries`;
    ...
    async function loadWarehouseBrief() {
        ...
        try {
            const response = await fetch(apiUrl('/api/runtime/warehouse-brief'));
            if (!response.ok) {
                throw new Error(`Warehouse brief request failed with ${response.status}`);
            }

            const payload = await readJsonOrThrow(response, 'Warehouse brief');
    ...
    async function loadQueryAuditFeed() {
        ...
        try {
            const response = await fetch(apiUrl('/api/query-audit/recent'));
            if (!response.ok) {
                throw new Error(`Query audit request failed with ${response.status}`);
            }

            const payload = await readJsonOrThrow(response, 'Query audit feed');
            const items = payload.items || [];
            if (items.length === 0) {
                renderReviewList(warehouseAuditFeed, ['No governed query requests recorded yet.']);
                warehouseAuditCount.innerText = '0 requests';
    ...
    async function loadQuerySessionBoard() {
        ...
        try {
            const response = await fetch(apiUrl('/api/query-session-board?limit=6'));
            if (!response.ok) {
                throw new Error(`Query session board request failed with ${response.status}`);
            }

            const payload = await readJsonOrThrow(response, 'Query session board');
            latestSessionBoardPayload = payload;
            const summary = payload.summary || {};
            const items = payload.items || [];
            renderDetailCard(sessionBoardSummary, [
    ...
    async function runPolicyCheck() {
        ...
        try {
            const response = await fetch(apiUrl('/api/policy/check'), {
                method: 'POST',
                headers: {
            ...
            const payload = await readJsonOrThrow(response, 'Policy check');
    ...
    async function loadGoldEvalRun() {
        ...
        try {
            const response = await fetch(apiUrl('/api/evals/nl2sql-gold/run'));
            if (!response.ok) {
                throw new Error(`Gold eval run failed with ${response.status}`);
            }
            const payload = await readJsonOrThrow(response, 'Gold eval');
            latestGoldEvalPayload = payload;
            const summary = payload.summary || {};
            const items = payload.items || [];
            const failing = items.filter((item) => item.status !== 'pass');
    ...
    async function copyArchitectureRoutes() {
        const routes = latestArchitectureRoutes.length > 0
            ? latestArchitectureRoutes
    ...
    async function copyGovernedClaim() {
        ...
        const lines = [
            ...
            '',
            'Fast routes',
            ...((latestArchitectureRoutes.length > 0 ? latestArchitectureRoutes : ['/api/architecture-pack', '/api/evals/nl2sql-gold/run', '/api/query-audit/recent'])
                .slice(0, 4)
                .map((item) => `- ${item}`)),
        ];
    ...
    async function copyQueryDecisionBrief() {
        ...
        const lines = [
            ...
            '',
            'Fast routes',
            ...((latestArchitectureRoutes.length > 0 ? latestArchitectureRoutes : ['/api/query-review-board', '/api/query-audit/recent', '/api/query-audit/{request_id}'])
                .slice(0, 4)
                .map((item) => `- ${item}`)),
        ];
        const ok = await copyTextToClipboard(lines.join('\n'));
    ...
    async function copyReviewBundle() {
        const bundle = [
            ...
            '',
            'Fast routes',
            ...((latestArchitectureRoutes.length > 0 ? latestArchitectureRoutes : ['/api/architecture-pack', '/api/query-audit/recent', '/api/evals/nl2sql-gold/run'])
                .map((route) => `- ${route}`)),
        ];
    ...
    function renderLensPanel() {
        ...
        `).join('');
        [lensAnalystBtn, lensArchitectureBtn, lensExecutiveBtn].forEach((btn) => btn?.classList.remove('active'));
        if (currentLens === 'analyst') lensAnalystBtn?.classList.add('active');
        if (currentLens === 'architecture') lensArchitectureBtn?.classList.add('active');
    ...
    copyArchitectureRoutesBtn.addEventListener('click', copyArchitectureRoutes);
    ...
    lensArchitectureBtn.addEventListener('click', () => { currentLens = 'architecture'; renderLensPanel(); });
