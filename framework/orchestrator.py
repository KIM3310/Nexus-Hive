...
- Router: LLM-based routing to select the right agent per request
"""
...
from framework.agent import BaseAgent, AgentContext
...
class Orchestrator:
    ...
    def __init__(self, memory: MemoryManager | None = None) -> None:
        ...
        self._routes: dict[str, str] = {}  # pattern → agent_name
    ...
    async def run_sequential(
        ...
        for agent in agents:
            output = await agent.run(current_input, request_id=request_id)
    ...
    async def run_parallel(
        ...
        """Run agents in parallel, each receiving the same input."""
        agents = self._resolve_agents(agent_names)
        start = time.time()

        tasks = [
            agent.run(input_data, request_id=request_id)
            for agent in agents
        ]
    ...
    async def run_routed(
        ...
        output = await selected_agent.run(input_data, request_id=request_id)
    ...
    def _collect_metrics(self, agents: list[BaseAgent]) -> dict[str, dict[str, Any]]:
        return {
            agent.name: {
                ...
                "elapsed_ms": agent._context.elapsed_ms if agent._context else 0,
